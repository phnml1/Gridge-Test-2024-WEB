...
const KakaoJoin = () => {
  ...
  console.log(userInfo);
   const join = async () => {
    try {
      await requestNotJwt.post('/auth/kakao/sign-up-by-token',userInfo);
      navigate('/login');
    }
    catch(error) {
      alert('회원가입에 실패하였습니다.');
  ...
  return (
    <JoinRoot>
      <JoinWrap>
        <JoinBox>
          {(boxTheme==='join') && (<JoinInputBox userInfo = {userInfo} setUserInfo = {setUserInfo} setBoxTheme={setBoxTheme} kakao = {true}/>)}
