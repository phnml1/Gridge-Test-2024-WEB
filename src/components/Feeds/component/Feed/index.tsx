...
const getComment = async (id: number, count: number) => {
  const response = await request.get(`/feeds/${id}/comments`, {
    params: {
      size: count,
      page: 1,
    },
...
const Feed = (props: FeedProps) => {
  ...
  const { data: commentData } = useQuery(
    ["comment", props.feed.id, props.feed.feedCommentCount],
    () => getComment(props.feed.id, props.feed.feedCommentCount),
    { enabled: props.feed.feedCommentCount > 0 }
  );
  useEffect(() => {
    setMoreText(props.feed.feedText.length <= 100);
  }, []);
  return (
    <Wrap>
      <ContentBackGround>
        ...
        <ContentSwiper
          setIndex={setIndex}
          buttonpos={15}
          contents={props.feed.contentList}
        />
      </ContentBackGround>
      <ContentWrap>
        <Icons>
          <HeartComment>
            {props.feed.isLiked ? (
              <Icon src={fillHeart} onClick={() => addLike(props.feed.id)} />
            ) : (
              <Icon
                src={feedHeart}
                onClick={() => addLike(props.feed.id)}
              ></Icon>
            )}
          ...
          {props.feed.isBookMarked ? (
            <BookMark
              src={fillBookmark}
              onClick={() => addBookmark(props.feed.id)}
            ></BookMark>
          ) : (
            <BookMark
              src={bookmark}
        ...
        <Content>
          ...
          <FeedText>
            ...
            <FeedContentSumContent>
              {moreText
                ? props.feed.feedText
                : truncateText(props.feed.feedText)}
              {!moreText && (
                <FeedMoreButton
                  onClick={() => {
                    setMoreText(true);
          ...
          {props.feed.feedCommentCount > 2 ? (
            <CommentButton
              onClick={() => {
                setModal(props.feed.id);
              }}
            >
              댓글 {props.feed.feedCommentCount}개 모두 보기
            </CommentButton>
          ) : (
            <CommentWrap>
              {commentData?.map((a: CommentType) => (
                <Comment
                  key={a.id}
                  id={a.writeUserLoginId}
                  text={a.commentText}
                />
              ))}
            </CommentWrap>
