...
import { DeleteModalButtonWrap, DeleteModalItem, DeleteModalItemDelete, DeleteModalWrap, Item, ItemDelete, ModalWrap, Outside, Question, ThisQuestion, Wrap } from "./styles";
...
const MoreModal = (props: MoreModalProps) => {
  ...
  return (<Wrap>
    ...
    {(theme === 'modal') && (<ModalWrap>
      <ItemDelete onClick={() => {setTheme('delete')}}>삭제</ItemDelete>
      <Item>수정</Item>
      <Item>좋아요 수 숨기기</Item>
      <Item>댓글 기능 해제</Item>
      <Item>게시물로 이동</Item>
    ...
    {
      (theme === 'delete') && (<DeleteModalWrap>
        <Question>게시물을 삭제하시겠습니까?</Question>
        <ThisQuestion>이 게시물을 삭제하시겠습니까?</ThisQuestion>
        <DeleteModalButtonWrap>
        <DeleteModalItemDelete onClick={() => {setDeleteAllowed(true)}}>삭제</DeleteModalItemDelete>
        <DeleteModalItem onClick={()=> setTheme('modal')}>취소</DeleteModalItem>
        </DeleteModalButtonWrap>
      </DeleteModalWrap>)
    }
