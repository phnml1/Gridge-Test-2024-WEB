...
import { AgreeBoxes, AllAgreeCheckBox, Guide, Title,JoinButtonWrap } from "./styles";
...
const AgreeBox = (props:AgreeBoxProps) => {
  ...
  return (<Fragment>
    ...
    <Guide>
    Tnovel은 회원님의 개인정보를 안전하게 보호합니다.<br/>
새 계정을 만드려면 모든 약관에 동의하세요.
    </Guide>
    <AllAgreeCheckBox>
      <CheckBox index={-1} check={isAllAgree()} setCheck={setAgree} content="이용약관 3개에 모두 동의"></CheckBox>
    </AllAgreeCheckBox>
    <AgreeBoxes>
      {agree.map((a,i)=> {
        return (<CheckBox index={i} check={a} setCheck={setAgree} content={agreeMessages[i]}></CheckBox>)
      })}
    </AgreeBoxes>
    <JoinButtonWrap>
    {isAllAgree()? (<JoinButton able={true} onClick={props.join}>가입</JoinButton>): (<JoinButton able={false}>가입</JoinButton>)}
