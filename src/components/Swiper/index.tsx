    ...
    <Wrap>
      ...
      >
        {props.contents?.map((a) => (
          <SwiperSlide style={{ width: "100%", height: "100%" }} key={a.id}>
            <Item src={a.contentUrl}></Item>
          </SwiperSlide>
        ))}
        {index !== 0 && <LeftButton buttonpos={props.buttonpos} />}
        {index !== props.contents.length - 1 && (
          <RightButton buttonpos={props.buttonpos} />
        )}
      </Swiper>
      {props.isModal && (
        <Indexes>
          {props.contents.map((a, i) =>
            index === i ? (
              <Index key={a.id}></Index>
            ) : (
              <EmptyIndex key={a.id}></EmptyIndex>
            )
          )}
        </Indexes>
      )}
